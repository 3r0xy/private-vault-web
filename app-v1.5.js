...
function requestFolderColor(path) {
  ...
  els.folderColorInput.value = state.folderColors[state.folderColorTarget] || "#8b5cf6";
  try {
...
async function relocateFolder(oldFolder, newFolder) {
  ...
  remapFolderColors(oldFolder, newFolder);
  await loadTree();
...
async function buildGraph(forceReload = false) {
  ...
  try {
    ...
    for (const note of state.notes) {
      if (note.path.startsWith("planner/")) continue;
...
const PLANNER_SECTIONS = [
  { id: "meetings", label: "Встречи и лекции" },
  { id: "general", label: "Общее" },
  { id: "health", label: "Здоровье и активность" },
];
...
function plannerParse(text, start) {
  ...
  for (const line of String(text || "").split(/\r?\n/)) {
    ...
    const sectionMatch = PLANNER_SECTIONS.find(x => line.startsWith(`### ${x.label}`));
...
async function plannerSaveNow(showFeedback = false) {
  ...
  if (!p.data || !p.path) return;
  if (p.saving) {
    p.pendingSave = true;
    while (p.saving) await new Promise(resolve => setTimeout(resolve, 60));
  ...
  try {
    ...
    const markdownText = plannerSerialize(p.data);
    const saved = await persistMarkdownFile({ path:p.path, oldPath:p.path, currentSha:p.sha, markdownText });
    p.sha = saved.sha;
    plannerRegisterFile(saved.path, saved.sha, markdownText);
    setSyncStatus("Планер сохранён");
    if (showFeedback) showToast("Планер сохранён в GitHub.");
  } catch (e) {
    p.dirty = true;
    setSyncStatus("Ошибка планера");
    showToast(`Планер не сохранён: ${e.message}`);
  } finally {
    p.saving = false;
    if (p.pendingSave || p.dirty) {
      p.pendingSave = false;
      clearTimeout(p.saveTimer);
      p.saveTimer = setTimeout(() => plannerSaveNow(false), 500);
    }
  }
}
function plannerScheduleSave() {
  ...
  clearTimeout(p.saveTimer);
  p.saveTimer = setTimeout(() => plannerSaveNow(false), 650);
}
async function plannerOpenWeek(start) {
  ...
  state.planner.weekStart = start;
  state.planner.path = path;
  state.planner.selectedDay = Math.max(0, Math.min(6, state.planner.selectedDay || 0));
  const found = await plannerFetchFile(path);
  if (found) {
    state.planner.sha = found.sha;
    state.planner.data = plannerParse(found.text, start);
    plannerRegisterFile(path, found.sha, found.text);
  } else {
    state.planner.sha = null;
...
function plannerCloneRecurring(source, targetStart) {
  ...
  for (const [key,t] of daily) {
    const recurrenceId = t.recurrenceId || plannerTaskId();
  ...
  for (const [key,t] of weekly) {
    tasks.push({ ...t, id:plannerTaskId(), recurrenceId:t.recurrenceId || plannerTaskId(), done:false, order:Date.now()+tasks.length });
  }
  return tasks;
}
async function plannerCreateWeek(start = state.planner.weekStart, carryTasks = [], sourceOverride = null) {
  start = start || plannerCurrentWeekStart();
  const existing = await plannerFetchFile(plannerWeekPath(start));
  if (existing) {
    await plannerOpenWeek(start);
    return;
  }
  const source = sourceOverride || await plannerPreviousData(start);
  const data = plannerEmptyData(start);
  data.tasks.push(...plannerCloneRecurring(source, start));
  for (const sourceTask of carryTasks) {
    data.tasks.push({ ...sourceTask, id:plannerTaskId(), recurrenceId:null, repeat:"none", done:false, order:Date.now()+data.tasks.length });
  }
  state.planner.weekStart = start;
  state.planner.path = plannerWeekPath(start);
  state.planner.sha = null;
  state.planner.data = plannerNormalizeData(data, start);
  plannerRender();
  await plannerSaveNow(true);
}
...
function plannerOpenTaskModal(day = 0, section = "general", taskId = null) {
  ...
  if (existing) {
    ...
    if (oldRepeat === "daily" && values.repeat === "daily") {
      for (const task of data.tasks.filter(t => t.recurrenceId === existing.recurrenceId || t.id === existing.id)) {
        task.text=values.text; task.section=values.section; task.time=values.time; task.duration=values.duration; task.repeat="daily"; task.recurrenceId=recurrenceId;
      }
      ...
      for (let day=0;day<7;day++) data.tasks.push({ id:plannerTaskId(),recurrenceId,day,section:values.section,text:values.text,time:values.time,duration:values.duration,repeat:"daily",done:day===values.day?existing.done:false,order:Date.now()+day });
    } else {
    ...
    for (let day=0;day<7;day++) data.tasks.push({ id:plannerTaskId(),recurrenceId,day,section:values.section,text:values.text,time:values.time,duration:values.duration,repeat:"daily",done:false,order:Date.now()+day });
  } else {
...
function plannerOpenReview() {
  ...
  if (!review.carry.length) html += `<div class="planner-review-note">Все уникальные задачи завершены. Повторяющиеся задачи появятся на следующей неделе автоматически.</div>`;
    ...
    for (const t of review.carry) html += `<label class="planner-carry-row"><input class="planner-carry-check" data-task-id="${escapeAttr(t.id)}" type="checkbox" checked><span>${escapeHtml(t.text)}</span><span>${PLANNER_DAY_SHORT[t.day]}</span></label>`;
  }
  html += `</div>`;
...
async function plannerFinalizeWeek() {
  ...
  const carryIds = new Set(Array.from(els.plannerReviewContent.querySelectorAll(".planner-carry-check:checked")).map(x=>x.dataset.taskId));
  const carry = review.carry.filter(t=>carryIds.has(t.id));
  try {
    setSyncStatus("Сохраняю итоги недели…");
    await plannerSaveNow(false);
    const statsPath = plannerStatsPath(data.weekStart);
    const existing = await plannerFetchFile(statsPath);
    const text = plannerStatsMarkdown(data,review);
    const saved = await persistMarkdownFile({path:statsPath,oldPath:statsPath,currentSha:existing?.sha||null,markdownText:text});
    plannerRegisterFile(statsPath,saved.sha,text);
    const next = plannerAddDays(data.weekStart,7);
    plannerCloseReview();
    await plannerCreateWeek(next,carry,data);
    await plannerOpenWeek(next);
    showToast("Итоги сохранены. Следующая неделя подготовлена.");
  } catch(e) {
    showToast(`Не удалось закрыть неделю: ${e.message}`);
  }
}
...
els.plannerPrevWeekBtn.onclick = () => plannerOpenWeek(plannerAddDays(state.planner.weekStart || plannerCurrentWeekStart(), -7));
els.plannerNextWeekBtn.onclick = () => plannerOpenWeek(plannerAddDays(state.planner.weekStart || plannerCurrentWeekStart(), 7));
els.plannerTodayBtn.onclick = () => plannerOpenWeek(plannerCurrentWeekStart());
els.plannerCreateWeekBtn.onclick = () => plannerCreateWeek(state.planner.weekStart || plannerCurrentWeekStart());
els.plannerReviewBtn.onclick = plannerOpenReview;
els.plannerOpenMarkdownBtn.onclick = plannerOpenMarkdown;
