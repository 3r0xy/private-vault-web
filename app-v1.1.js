...
function scheduleDerivedDocumentUI(text) {
  ...
  derivedUiTimer = setTimeout(() => {
    ...
    renderMediaPreview(text, els.pathInput.value.trim() || state.current || "", els.mediaPreview);
  }, 140);
}
...
class TaskInlineWidget extends WidgetType {
  ...
  toDOM(view) {
    const span = document.createElement("span");
    span.className = `task-inline-widget ${this.checked ? "checked" : ""}`;
    span.setAttribute("role", "checkbox");
    span.setAttribute("aria-checked", this.checked ? "true" : "false");
    span.title = this.checked ? "Снять отметку" : "Отметить задачу";
      ...
      if (!seenLines.has(line.number)) {
        ...
        while ((wm = wikiRe.exec(text)) !== null) {
          ...
          ranges.push(Decoration.mark({
            ...
            attributes: { "data-wiki": rawWiki, title: resolved ? `Открыть [[${rawWiki}]]` : `Создать [[${normalizeWikiTarget(rawWiki)}]]` }
          }).range(from + 2, to - 2));
        ...
        while ((im = italicRe.exec(text)) !== null) {
          ...
          if (!overlapsWiki(from, to)) {
            ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(from, from + 1));
            ranges.push(Decoration.mark({ class: "cm-live-italic" }).range(from + 1, to - 1));
            ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(to - 1, to));
          }
        }
        ...
        while ((ulm = underlineRe.exec(text)) !== null) {
          ...
          ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(from, from + 3));
          ranges.push(Decoration.mark({ class: "cm-live-underline" }).range(from + 3, to - 4));
          ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(to - 4, to));
        }
        ...
        while ((lm = linkRe.exec(text)) !== null) {
          ...
          if (!overlapsWiki(from, to)) {
            ...
            ranges.push(Decoration.mark({ class: "cm-live-link", attributes: { "data-href": lm[2], title: `Открыть ${lm[2]}` } }).range(labelFrom, labelTo));
        ...
        if (taskMatch) {
          ...
          ranges.push(Decoration.replace({
            widget: new TaskInlineWidget(/[xX]/.test(taskMatch[3]), checkPos)
    ...
    extensions: [
      ...
      EditorView.updateListener.of(update => {
        ...
        if (update.docChanged) {
          ...
          renderMediaPreview(value, els.secondaryPathInput.value.trim() || state.secondary.path || "", els.secondaryMediaPreview);
        }
      }),
      EditorView.domEventHandlers({
...
function setSecondaryMarkdown(text) {
  ...
  renderMediaPreview(text, els.secondaryPathInput.value.trim() || state.secondary.path || "", els.secondaryMediaPreview);
}
