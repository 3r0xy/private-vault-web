...
const els = {
  ...
  imageInput: $("imageInput"), highlightColorInput: $("highlightColorInput"),
};
      ...
      if (!seenLines.has(line.number)) {
        ...
        const base = line.from;
        {
          ...
          while ((wm = wikiRe.exec(text)) !== null) {
            ...
            ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(from, from + 2));
            ranges.push(Decoration.mark({
              class: `cm-live-wiki-target ${resolved ? "" : "missing"}`,
              attributes: { "data-wiki": rawWiki, title: resolved ? `Ctrl/Cmd + клик: открыть [[${rawWiki}]]` : `Ctrl/Cmd + клик: создать [[${normalizeWikiTarget(rawWiki)}]]` }
            }).range(from + 2, to - 2));
            ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(to - 2, to));
          }
          ...
          const colorHighlightRe = /<mark\s+style=["']background-color:\s*(#[0-9a-fA-F]{6})["']>([^\n]*?)<\/mark>/gi; let chm;
          while ((chm = colorHighlightRe.exec(text)) !== null) {
            const from = base + chm.index, openLen = chm[0].indexOf(">") + 1, innerFrom = from + openLen, innerTo = innerFrom + chm[2].length, to = from + chm[0].length;
            ranges.push(Decoration.mark({ class: "cm-md-marker cm-color-markup" }).range(from, innerFrom));
            if (innerFrom < innerTo) ranges.push(Decoration.mark({ class: "cm-live-color-highlight", attributes: { style: `background-color:${chm[1]}` } }).range(innerFrom, innerTo));
            ranges.push(Decoration.mark({ class: "cm-md-marker cm-color-markup" }).range(innerTo, to));
          }
          ...
          while ((lm = linkRe.exec(text)) !== null) {
            ...
            if (!overlapsWiki(from, to)) {
              ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(from, labelFrom));
              ranges.push(Decoration.mark({ class: "cm-live-link", attributes: { "data-href": lm[2], title: `Ctrl/Cmd + клик: ${lm[2]}` } }).range(labelFrom, labelTo));
              ranges.push(Decoration.mark({ class: "cm-md-marker" }).range(labelTo, to));
            }
          }
          ...
          if (taskMatch) {
            ...
            ranges.push(Decoration.mark({
              class: `cm-task-marker ${/[xX]/.test(taskMatch[3]) ? "checked" : ""}`,
              attributes: { "data-check-pos": String(checkPos), role: "checkbox", "aria-checked": /[xX]/.test(taskMatch[3]) ? "true" : "false", title: "Отметить задачу" }
            }).range(markerFrom, markerTo));
          } else {
            const bulletMatch = text.match(/^(\s*)([-*+])(\s+)/);
            if (bulletMatch) {
              const markerFrom = base + bulletMatch[1].length;
              const markerTo = markerFrom + bulletMatch[2].length;
              ranges.push(Decoration.line({ class: "cm-live-list-line" }).range(base));
              ranges.push(Decoration.mark({ class: "cm-bullet-marker" }).range(markerFrom, markerTo));
...
function handleLiveEditorPointer(event, view, pane = "primary") {
  const task = event.target?.closest?.(".cm-task-marker");
  if (task) {
    event.preventDefault();
    event.stopPropagation();
    const pos = Number(task.dataset.checkPos);
    if (Number.isFinite(pos)) {
      const current = view.state.doc.sliceString(pos, pos + 1);
      view.dispatch({ changes: { from: pos, to: pos + 1, insert: /[xX]/.test(current) ? " " : "x" } });
...
async function saveCurrent() {
  ...
  const tab = activeTab();
  const requestedPath = (els.pathInput.value.trim() || tab?.path || "").trim();
...
function applyHighlightColor(color) {
  ...
  const insert = `<mark style="background-color:${color}">${selected}</mark>`;
  const innerStart = ctx.start + insert.indexOf(">") + 1;
...
function insertExternalLink(pane = "primary") {
  ...
  const label = selected || prompt("Текст ссылки", "ссылка");
  if (!label) return;
...
async function buildGraph(forceReload = false) {
  ...
  try {
    // Refresh file metadata when requested, then merge saved notes with currently
    // open tabs so every visible draft can participate in the graph as well.
    if (forceReload) await loadTree();
    const entries = new Map();
    for (const note of state.notes) entries.set(note.path, { path: note.path, saved: true, text: null });
    for (const tab of state.tabs) {
      if (!tab.path || !tab.path.toLowerCase().endsWith(".md")) continue;
      const prev = entries.get(tab.path) || { path: tab.path, saved: !tab.isNew, text: null };
      prev.text = tab.text ?? prev.text;
      prev.saved = prev.saved || !tab.isNew;
    ...
    for (const note of all) {
      ...
      if (text == null) {
        try {
          const item = forceReload ? await fetchFileFresh(note.path) : await getFile(note.path);
          text = item.text;
        } catch {
          text = "";
        }
    ...
    for (const link of links) {
      nodesMap.get(link.source) && nodesMap.get(link.source).degree++;
      nodesMap.get(link.target) && nodesMap.get(link.target).degree++;
    }
...
function renderGraph(nodes, links) {
  ...
  nodes.forEach((d, i) => {
    d._phase = (i * 1.37 + (d.id?.length || 1)) % (Math.PI * 2);
    d._amp = 0.32 + (i % 3) * 0.08;
    d._dragMoved = false;
  });
  ...
  const simulation = d3.forceSimulation(nodes)
    .force("link", d3.forceLink(links).id(d => d.id).distance(92).strength(.34))
    .force("charge", d3.forceManyBody().strength(d => -115 - Math.min(165, (d.degree || 0) * 9)))
    .force("center", d3.forceCenter(width / 2, height / 2))
    .force("x", d3.forceX(width / 2).strength(.004))
    .force("y", d3.forceY(height / 2).strength(.004))
    .force("collision", d3.forceCollide().radius(d => 25 + Math.min(20, (d.degree || 0) * 1.6)).strength(.9))
    .velocityDecay(.48)
    .alphaDecay(.045);
  ...
  const draw = t => {
    ...
    for (const d of nodes) {
      ...
      const drift = d._dragging ? 0 : d._amp;
      positions.set(d.id, [x + Math.sin(t * .00022 + d._phase) * drift, y + Math.cos(t * .00019 + d._phase * 1.2) * drift]);
    }
    link
      .attr("x1", d => positions.get(d.source.id || d.source)?.[0] ?? d.source.x)
    ...
    node.attr("transform", d => {
      const [x, y] = positions.get(d.id) || [d.x, d.y];
      return `translate(${x},${y})`;
    });
    state.graph.ambientFrame = requestAnimationFrame(draw);
  };
  state.graph.ambientFrame = requestAnimationFrame(draw);

  const localPointer = event => d3.pointer(event.sourceEvent || event, root.node());
  const drag = d3.drag()
    .clickDistance(5)
    .on("start", (event, d) => {
      d._dragging = true;
      ...
      d._dragStartX = x; d._dragStartY = y;
      d.fx = Number.isFinite(d.x) ? d.x : x;
      d.fy = Number.isFinite(d.y) ? d.y : y;
      if (!event.active) simulation.alphaTarget(.06).restart();
    })
    .on("drag", (event, d) => {
      const [x, y] = localPointer(event);
      if (Math.hypot(x - d._dragStartX, y - d._dragStartY) > 3) d._dragMoved = true;
      d.fx = x; d.fy = y; d.x = x; d.y = y;
    })
    .on("end", (event, d) => {
      d._dragging = false;
      d._pinned = true;
      ...
      setTimeout(() => { d._dragMoved = false; }, 120);
    });
  node.call(drag);
